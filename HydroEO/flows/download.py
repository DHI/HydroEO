...
class RiverDownloadFlow:
    ...
    Note: Rivers currently only support SWOT Hydrocron (public HTTP API),
...
@dataclass
class SWOTRasterDownloadFlow:
    """Run SWOT raster downloads for arbitrary areas of interest.

    Requires earthaccess credentials for querying and downloading SWOT
    granules from NASA's DAAC.
    """

    swot_raster_config: dict[str, Any]
    project_dir: str
    earthdata_credentials: tuple[str | None, str | None]

    def run(self) -> None:
        """Execute SWOT raster download, preprocessing, and cleanup.

        Uses earthaccess credentials to query and download SWOT granules.
        Extracts specified layers, clips to AOI, and cleans up raw downloads.
        """
        logger.info(
            "SWOT Raster Download - AOI: %s, Product: %s, Temporal range: %s to %s",
            self.swot_raster_config["aoi"]["name"],
            self.swot_raster_config["product"],
            self.swot_raster_config["startdate"],
            self.swot_raster_config["enddate"],
        )

        # Setup directories
        aoi_name = self.swot_raster_config["aoi"]["name"]
        product = self.swot_raster_config["product"]
        base_dir = Path(self.project_dir) / "swot_raster" / aoi_name
        raw_dir = base_dir / "raw" / product
        processed_dir = base_dir / "processed" / product
        raw_dir.mkdir(parents=True, exist_ok=True)
        processed_dir.mkdir(parents=True, exist_ok=True)

        # Setup download tracking log
        log_path = raw_dir / "downloaded.log"
        if not log_path.exists():
            log_path.touch()

        # Read already processed granules
        with open(log_path, "r") as log_file:
            processed_granules = {line.rstrip() for line in log_file}

        # Download phase
        downloaded_files = self._download_granules(raw_dir, processed_granules)
        if downloaded_files:
            # Preprocess phase: extract layers and clip to AOI only if new files downloaded
            self._preprocess_granules(
                raw_dir, processed_dir, log_path, downloaded_files
            )
        else:
            logger.info("No new granules to download")

        # Merge and reproject phase: organize by date and merge tiles
        # This runs regardless of whether new granules were downloaded
        # (allows catching up on merge if it was interrupted or skipped in previous run)
        ...
        if processed_files:
            ...
            self._merge_and_reproject_granules(processed_dir)
        else:
    ...
    def _download_granules(
        ...
        # Parse AOI bounds
        aoi_config = self.swot_raster_config["aoi"]
        ...
        startdate = self.swot_raster_config["startdate"]
        enddate = self.swot_raster_config["enddate"]
        time_start = (
            f"{startdate[0]:04d}-{startdate[1]:02d}-{startdate[2]:02d} 00:00:00"
        )
        time_end = f"{enddate[0]:04d}-{enddate[1]:02d}-{enddate[2]:02d} 23:59:59"
        logger.info("Temporal range: %s to %s", time_start, time_end)

        granule_filter = self.swot_raster_config.get("granule_filter", None)
        if granule_filter:
        ...
        # Filter to granules not yet processed
    ...
    def _preprocess_granules(
        ...
        # Default variables to extract (can be extended via config)
        variables = [
            "wse",
            "wse_uncert",
            "wse_qual",
        ...
        # Load AOI if provided (for clipping)
        aoi_gdf = None
        ...
        # Process each downloaded netCDF file
        ...
        for nc_path in tqdm(nc_files, desc="Processing netCDF files"):
            ...
            # Detect CRS
            native_crs = self._detect_crs(ds, nc_path)
            ...
            for var in tqdm(
                variables, desc=f"Variables in {nc_path.stem}", leave=False
            ):
                if var not in ds:
                    continue
        ...
        # Update log file with processed granules
        nc_names = [f.stem for f in nc_files]
        with open(log_path, "a") as log_file:
            for name in nc_names:
                log_file.write(name + "\n")

        # Cleanup: remove raw downloaded files
    ...
    def _detect_crs(self, ds: xr.Dataset, nc_path: Path) -> CRS | None:
        ...
        # Try metadata in CRS-related variables
        ...
        # Try to get CRS from data variable rio accessor
        ...
        # Try to extract UTM zone from filename
    ...
    def _merge_and_reproject_granules(self, processed_dir: Path) -> None:
        """Organize processed TIFFs by date and merge/reproject tiles."""
        logger.info("=== SWOT Raster Merge and Reproject Phase ===")

        # Get target CRS from config or use global CRS
        target_crs = self.swot_raster_config.get("target_crs", None)
        if not target_crs:
        ...
        # Create merged output directory (sibling of processed, not nested)
        ...
        # Group files by variable and date
        file_groups = {}  # {(variable, date): [list of files]}
        for tif_path in tif_files:
            # Extract date from filename (e.g., "20231004" from "...20231004T225225_wse.tif")
                ...
                with rasterio.open(dst_path, "w", **profile) as dst:
                    for b in range(1, src.count + 1):
                        reproject(
                            ...
                            dst_crs=dst_crs,
                            resampling=self._resampling_for(var_name),
                        )
        except Exception as e:
            logger.warning("Failed to reproject %s: %s", src_path.name, e)

    def _merge_rasters(
        self, tif_list: list[Path], dst_path: Path, epsg: int, var_name: str
    ) -> None:
        ...
        try:
            # Reproject all inputs to temp files
            for i, tif_path in enumerate(tif_list):
                tmp_out = Path(tmpdir) / f"reproj_{i}.tif"
                self._reproject_raster(tif_path, tmp_out, epsg, var_name)
                reproj_paths.append(tmp_out)

            # Open reprojected datasets and merge
    ...
    def _resampling_for(self, var_name: str) -> Resampling:
        """Choose appropriate resampling method based on variable type."""
        var_lower = var_name.lower()
        # Continuous fields -> bilinear (exact match or with underscore prefix)
        continuous_exact = {"wse", "wse_uncert", "geoid", "height_cor_xover"}
        if var_lower in continuous_exact:
            return Resampling.bilinear
        # Counts/masks/quality -> nearest
