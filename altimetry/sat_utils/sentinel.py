...
def subset(aoi: list, download_dir: str, dest_dir: str, file_id: str ='enhanced_measurement.nc', product: str='S3'):
    """
    ...
    VARIABLES = {
        'S3' : ['lat_20_ku', 'lon_20_ku', 'elevation_ocog_20_ku', 'waveform_20_ku', 'sig0_ocog_20_ku','time_20_ku', 'alt_20_ku', 'tracker_range_20_ku', 'range_ocog_20_ku', 'lat_01', 'geoid_01'],
        'S6' : ['latitude', 'longitude', 'altitude', 'range_ocog', 'model_wet_tropo_cor_measurement_altitude', 'model_dry_tropo_cor_measurement_altitude', 'sig0_ocog', 'time_tai', 'geoid']
    }

    ATTRIBUTES = {
        'S3' : ['first_meas_lat', 'last_meas_lat', 'pass_number'],
        'S6' : ['first_measurement_latitude', 'last_measurement_latitude', 'pass_number']
    }
    ...
    for folder in os.listdir(download_dir):
        if folder.endswith(EXTENSION[product]):
            ...
            if os.path.isfile(file):
                ...
                nc_cropped = os.path.join(dest_dir, 'sub_' + os.path.split(folder)[-1].split(EXTENSION[product])[0]+'.nc')
                ...
                selected = np.where((lon20 <= lrx) & (lon20 >= ulx) &
                                    (lat20 >= lry) & (lat20 <= uly))[0]
                ...
                selected01 = np.where((lon01 <= lrx) & (lon01 >= ulx) &
                                        (lat01 >= lry) & (lat01 <= uly))[0]
                ...
                if max_index20 > 0:
                    with netCDF4.Dataset(file) as src, netCDF4.Dataset(nc_cropped, "w") as dst:
                        ...
                        if product == 'S3':
                            __crop_s3(src, dst, (min_index20, max_index20), (min_index01, max_index01))
                        
                        elif product == 'S6':
                            __crop_s6(src, dst, (min_index20, max_index20), (min_index01, max_index01))
...
def __crop_s3(src, dst, index_bounds_20, index_bounds_01):
    ...
    for name, dimension in src.dimensions.items():
        if '20_' in name:
            dst.createDimension(
                name, (len(np.arange(min_index20, max_index20+1, 1))
                        if not dimension.isunlimited() else None))
        elif '01' in name:
            dst.createDimension(
                name, (len(np.arange(min_index01, max_index01+1, 1))
                        if not dimension.isunlimited() else None))
        else:
    ...
    for name, variable in src.variables.items():
        if '20_' in name:
            dst.createVariable(name, variable.datatype,
                                variable.dimensions)
            dst[name].setncatts({k: variable.getncattr(k)
    ...
    for gr_name, group in src.groups.items():
        ...
        if '_01' in gr_name:
            ...
            for name, variable in src[gr_name].variables.items():
                dst[gr_name].createVariable(name, variable.datatype,
                                    variable.dimensions)
                dst[gr_name][name].setncatts({k: variable.getncattr(k)
                                        for k in variable.ncattrs()})
                dst[gr_name][name][:] = src[gr_name][name][min_index01:max_index01+1]
            ...
            for gr_name_bw, group in src[gr_name].groups.items():
                ...
                for name, variable in src[gr_name][gr_name_bw].variables.items():
                    dst[gr_name][gr_name_bw].createVariable(name, variable.datatype,
                                        variable.dimensions)
                    dst[gr_name][gr_name_bw][name].setncatts({k: variable.getncattr(k)
                                        for k in variable.ncattrs()})
                    dst[gr_name][gr_name_bw][name][:] = src[gr_name][gr_name_bw][name][min_index01:max_index01+1]
            ...
            for gr_name_bw, group in src[gr_name].groups.items():
                ...
                for name, variable in src[gr_name][gr_name_bw].variables.items():
                    ...
                    dst[gr_name][gr_name_bw][name].setncatts({k: variable.getncattr(k)
                                        for k in variable.ncattrs()})
                    dst[gr_name][gr_name_bw][name][:] = src[gr_name][gr_name_bw][name][min_index20:max_index20+1]

        else:
            for name, dimension in src[gr_name].dimensions.items():
                dst[gr_name].createDimension(name, (len(dimension)
                                        if not dimension.isunlimited()
                                        else None))
...
class Sentinel3:
    ...
    def __post_init__(self):
        ...
        with netCDF4.Dataset(self.infile, 'r') as src:
            ...
            self.epoch_OCOG = ((-self.tracker_range + self.range_OCOG - 0.55)*128/60 + 43) # 128 bins for 60m

            # set date attribute
            self.__setattr_from_file(src, 'tai' , 'time_20_ku')
            self.date = [(datetime.datetime(2000, 1, 1) + datetime.timedelta(seconds=c2_time)) for c2_time in self.tai]
    ...
    def __interpolate_geoid(self):
        ...
        if len(self.geoid_01) > 1:
            ...
            if self.lat_01[0] > self.lat_01[1]:
                geoid_interp = np.interp(self.lat, np.flip(self.lat_01, axis=0), np.flip(self.geoid_01, axis=0))
    ...
    def read(self) -> pd.DataFrame:
        ...
        vs['sat_path'] = np.repeat('descending' if self.first_meas_lat  - self.last_meas_lat > 0 else 'ascending', len(self.height))
        ...
        vs['orbit'] = np.repeat(list(filter(None, file_name_split))[10], len(self.height))
        vs['relative_orbit'] = np.repeat(list(filter(None, file_name_split))[9], len(self.height))
        vs['platform'] = np.repeat(list(filter(None, file_name_split))[1], len(self.height))
